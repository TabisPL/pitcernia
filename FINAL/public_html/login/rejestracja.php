...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nazwa_uzytkownika) || empty($email) || empty($haslo)) {
        ...
        if ($liczba_rekordow > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO Uzytkownicy (NazwaUzytkownika, Imie, Nazwisko, Email, HasloHash, DataRejestracji, CzyAktywny) VALUES (?, ?, ?, ?, ?, NOW(), FALSE)");
            ...
            if ($stmt->execute()) {
                ...
                $token_akt = bin2hex(random_bytes(16)); 
                $token_akt_hash = hash("sha256", $token_akt); 
                $wyg = date("Y-m-d H:i:s", time() + 60 * 60); 

                
                $stmt = $conn->prepare("UPDATE Uzytkownicy SET Token_akt = ?, Token_akt_wyg = ? WHERE UzytkownikID = ?");
                $stmt->bind_param('ssi', $token_akt_hash, $wyg, $uzytkownik_id);
                $stmt->execute();
                $stmt->close();

                // Wysyłanie e-maila z linkiem aktywacyjnym
                $link = "https://srv82461.seohost.com.pl/login/aktywacja.php?Token_akt=" . $token_akt_hash; // link aktywacyjny
                $temat = "Aktywacja konta";
                $tresc = "Cześć " . $imie . ",\n\nDziękujemy za rejestrację! Kliknij w poniższy link, aby aktywować swoje konto:\n\n" . $link;
                $naglowek = "From: srv82461@srv82461.seohost.com.pl";

                mail($email, $temat, $tresc, $naglowek);
                echo '
                <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
                <div class="bg-dark text-white" style="min-height: 100vh; padding-top: 10%; padding-left: 14%;">
                    <h2 class="mb-3">Na Twoj adres e-mail zostal wyslany link aktywacyjny.</h2>
                    <h2>Prosze sprawdzic skrzynke odbiorcza.</h2>
                </div>
';
...
<?php include '../navbar.php'; ?> <!-- TUTAJ NAVBAR INCLUDE JEST (komentarz dla mnie jakbym zgubil go) -->
    <main>
        <section class="form-container">
            ...
            <div id="register" class="form-section p-4 border rounded bg-secondary shadow text-center">
