...
if (isset($_GET['action']) && $_GET['action'] === 'markCompleted' && isset($_GET['id'])) {
  ...
  if ($baza->query($sql) === TRUE) {
      header("Location: szczegoly.php?view=active");
      exit;
  } else {
      ...
      <div class="">
        <div class="">
          ...
          if ($view === 'active') {
              ...
              // aktywne zamówienia
              $sql = "SELECT `z`.`ZamowienieID` AS orderId, `u`.`NazwaUzytkownika` AS username, `z`.`KwotaCalkowita`, `z`.Status, `z`.`DataUtworzenia`
                      FROM Zamowienia z
              ...
              if ($result && $result->num_rows > 0) {
                  ...
                  while ($order = $result->fetch_assoc()) {
                      ...
                      echo "<td>
                              <a href=\"szczegoly.php?view=details&id=" . urlencode($order['orderId']) . "\">Szczegóły</a> | 
                              <a href=\"ukonczone.php?action=markCompleted&id=" . urlencode($order['orderId']) . "\">Oznacz jako wykonane</a>
                            </td>";
                      echo "</tr>";
                  }
                  echo "</table>";
